...
typedef enum { VCAM_PIXFMT_RGB24 = 0x01, VCAM_PIXFMT_YUYV = 0x02, VCAM_PIXFMT_RGB24_X_COMPRESSOR = 0x05, VCAM_PIXFMT_YUYV_X_COMPRESSOR = 0x06 } pixfmt_t;

struct vcam_device_spec {
    unsigned int idx;
    unsigned short width, height;
    pixfmt_t pix_fmt;
    bool enable_x_compressor;
    char video_node[64];
    char fb_node[64];
};
