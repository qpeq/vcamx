...
extern unsigned char enable_x_compressor;

struct __attribute__((__packed__)) rgb_struct {
    unsigned char r, g, b;
};

// For storing the size of the compressed data ;
const u8 RESERVED_SIZE_BYTES = 4;
...
void compress_buffer(void* buf, size_t size)
{
    void* tmp_buf = vmalloc(size);
    if (NULL == tmp_buf) {
        pr_err("vmalloc for tmp_buf failed.\n");
    }
    else {
        x_init();
                                  ...
                                  struct vcam_device *dev)
{
    ...
    if (dev->enable_x_compressor) {
        compress_buffer(vbuf, size);
    }
                               ...
                               struct vcam_device *dev)
{
    ...
    if (dev->enable_x_compressor) {
        compress_buffer(out_vbuf_ptr, dev->output_format.sizeimage);
    }
                                       ...
                                       struct vcam_device_spec *dev_spec)
{
    ...
    *vdev = vcam_video_device_template;
    ...
    vcam->enable_x_compressor = (bool) enable_x_compressor;
    ...
    if (dev_spec->enable_x_compressor) {
        pr_debug("x-compressor enabled\n");
    }
    vcam->enable_x_compressor = dev_spec->enable_x_compressor;
