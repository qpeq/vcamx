...
int list_devices()
{
    ...
    while (!ioctl(fd, VCAM_IOCTL_GET_DEVICE, &dev)) {
        ...
        if (dev.enable_x_compressor) {
            printf("%d. %s(%d,%d,%s,x-compressor) -> %s\n", dev.idx, dev.fb_node, dev.width,
...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        case 'd':
            ...
            break;
        case 'x':
            printf("Enable x-compressor\n");
            dev.enable_x_compressor = true;
            break;
        }
    } while (next_option != -1);
